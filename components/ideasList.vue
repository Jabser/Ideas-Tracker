...
<template>
  <section>
    <article>
      <h4 class="text-5xl font-semibold mb-4">Latest Ideas</h4>
      <ul>
          ...
          <li class="my-6 py-6 border border-b-0 border-x-0 border-t-1" v-for="idea in ideas.current.value">
            <div>
              ...
              <p class="">{{ idea.description }}</p>
                <button class="mt-6 text-sm font-semibold leading-6 py-3 px-7 text-white bg-red-600 hover:bg-red-800 transition ease-in-out duration-300 rounded-full" aria-label="Remove item" v-if="user.current.value &&
